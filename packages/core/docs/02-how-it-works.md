...
- **頻度順短縮**（`minifyClassNames: true` で opt-in、本番ビルドのみ）: 全クラス確定後、使用頻度順に `a`, `b`, ... へ全単射リネームされる。HTML の class 属性が大幅に縮む（ベンチで -48%）。既定で無効なのは、短縮名がバンドル全体の頻度順で決まり、コンポーネントを 1 つ足すだけで既存クラスの名前がずれるため（内容ハッシュ名はビルドを跨いで安定する）
