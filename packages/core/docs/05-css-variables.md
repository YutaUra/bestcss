...
なお、動的スタイルの手段は CSS 変数だけではない（状態セレクタ・クラス合成・data 属性との使い分けは [動的なスタイルの 4 パターン](./06-dynamic-styles.md) を参照）。静的なトークン（`--color-primary`）と動的な値（`--progress`）が同じ仕組みに乗るため、「どこまでがビルド時でどこからが実行時か」を css`` の中で意識する必要がない。動的側にだけフォールバックを書いておくと、注入漏れでもレイアウトが壊れない。
