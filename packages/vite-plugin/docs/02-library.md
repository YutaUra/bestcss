...
Vite の lib モードでビルドする。**クラス名の短縮は無効のまま**にする（既定で無効なので指定は不要だが、意図を明示するために書いておくとよい）:
...
短縮してはいけない理由: 短縮名（a, b, c...）はビルドローカルな頻度順で、ライブラリが短縮名を出荷すると利用側アプリの短縮名と衝突する。**bc 名は内容ハッシュ（内容アドレス）なので衝突せず**、最終的な短縮は利用側ビルドに委ねられる。
...
- **ライブラリのクラス名も短縮対象**: 利用側が `minifyClassNames: true` でビルドすると、CSS アセットのセレクタからも bc 名を収穫するため、ライブラリ由来のクラスも a, b, c... へ短縮され、JS と CSS の一貫性が保たれる（webpack 利用側でも `BestCssWebpackPlugin` が同じことを行う）
